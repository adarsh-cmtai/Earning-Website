...
import { Users, ArrowRight, CheckCircle, Copy, DollarSign, Download, Settings, HelpCircle, Mail, TrendingUp, Clock, Megaphone, TrendingDown, Gift, Lock } from "lucide-react";
      ...
      <div className="grid gap-6 md:grid-cols-3 lg:grid-cols-5">
        ...
        <div className="md:col-span-1 lg:col-span-2 space-y-6">
          ...
          <Card className="shadow-sm"><CardHeader><CardTitle>AI Video Status</CardTitle></CardHeader>
            <CardContent>
              {aiVideo?.current ? (<div className="p-3 rounded-md bg-green-50 dark:bg-green-500/10 border"><div className="flex items-center gap-3"><CheckCircle className="h-5 w-5 text-green-600 flex-shrink-0" /><div><h4 className="font-semibold text-green-800 dark:text-green-300">New Video Available</h4><p className="text-sm text-green-700 dark:text-green-400 truncate">{aiVideo.current.title}</p></div><Button size="sm" asChild><a href={aiVideo.current.fileUrl} target="_blank" rel="noreferrer"><Download className="mr-2 h-4 w-4"/>Download</a></Button></div></div>) : (<p className="text-sm text-center py-2 text-muted-foreground">No new video is assigned.</p>)}
